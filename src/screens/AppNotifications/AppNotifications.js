...
const AppNotifications = () => {
    ...
    const clearHandler = () => {
        firestore().collection("users").doc(user?.uid).collection('notifications').get().then((snapshot) => {
    ...
    return (
        ...
        >
            ...
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', margin: 5 }}>
                ...
                <Button disabled={notifications?.length === 0} appearance='ghost' status='primary' onPress={clearHandler}>
                    CLEAR ALL
                    <View style={{ paddingLeft: 5 }}>
                        <FontAwesomeIcon icon={faCheckDouble} size={17} color={notifications?.length === 0 ? '#D3D3D3' : '#3366FF'} />
                    </View>
                </Button>
            </View>
            {loading ? (
                ...
                >
                    <SkeletonContent
                        ...
                        layout={[
                            { width: 350, height: 80, marginBottom: 10, borderRadius: 10 },
                            { width: 350, height: 80, marginBottom: 10, borderRadius: 10 },
                            { width: 350, height: 80, marginBottom: 10, borderRadius: 10 },
                            { width: 350, height: 80, marginBottom: 10, borderRadius: 10 },
                            { width: 350, height: 80, marginBottom: 10, borderRadius: 10 },
                ...
                <View style={{ flex: 1 }}>
                    {notifications?.length !== 0 ? <FlatList
                        data={notifications}
                        renderItem={({ item }) => <ListNotification
                            notification={item}
                        />}
                    /> :
